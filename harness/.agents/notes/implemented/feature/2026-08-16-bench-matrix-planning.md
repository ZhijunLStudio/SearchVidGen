...
- bench 逐格串行执行时经验记忆（_memory.jsonl）跨格共享——跨格学习是特性
  ...
  可能受益于前格经验。
- 已知成本（已缓解）：逐格执行时每格曾重新加载生成模型（首次真实执行实测
  每格含 ~5 分钟加载，E19）。2026-08-16 起 bench 用 `adapters_cache`
  （键 = name+params 归一化，复用范围由 bench 进程拥有）跨格复用相同参数
  的适配器实例——同参数消融（如 ratio 轴）从第 2 格起不再付加载成本；
  参数不同的轴（如 steps）仍新建实例。缓存不进全局状态，单次 run 语义不变。
