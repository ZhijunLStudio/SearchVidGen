...
- **裁判口径差异（E24 警报，E25 部分修正）**：E24 观察到的 vLLM ≈9-10 vs
  ...
  **可生成性 -1.5**（deepseek-text 更宽松）。混用裁判的 leaderboard
  ...
  "不可比" 收窄到"可标注修正"。
- 优化闭环的 ROI 有了基线（E26 修正后）：**JSON 模式修复解析后，off 基线
  2.65 → 7.94（E24 的 "+2.81 优化增益"绝大部分是解析修复被误记）**；
  同温候选 on == off（7.94，多样性不足零增益）；温度轮转（0.6/0.9/1.2）
  on = 8.06（+0.12，4× 成本）。当前预算下优化 ROI 弱，天花板有限
  （单次生成已 7.9/10）；预算调参是微调空间，不是故事主线。
- E35 修正：预算消融显示 **(3,3) 档 on=8.33（+0.52）**——预算调参不是
  纯微调；early-stop（target 8.0）使高预算成本可控（18 调用/3 试）。
  story.yaml 默认预算升为 3×3/target 8.0。
