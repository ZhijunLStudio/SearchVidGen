...
决策的"为什么"在 `.agents/notes/`（Agent Notes，46 篇）；证据在
`README.md` 的 E1-E46；本表只做对照。
...
| Agent Notes 决策记忆 | .agents/notes/{proposed,implemented}/（46 篇）+ README 格式规范 | 本仓库的 52 轮演进全靠它沉淀 |
| 实验即证据（evidence-driven） | experiments/（events+manifest+eval）+ E1-E46 + leaderboard 基线 | E8/E10 衔接结论；E21 修复路径复现 |
...
52 轮（2026-08-14 → 08-17）每轮 = 差距分析 → 修复/增强 → 测试 → 真实验证
