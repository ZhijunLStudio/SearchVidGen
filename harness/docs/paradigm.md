...
决策的"为什么"在 `.agents/notes/`（Agent Notes，30+ 篇）；证据在
`README.md` 的 E1-E29；本表只做对照。
...
| fail loud：配置错误最早点响亮失败 | schema 校验/instantiate 参数校验/能力校验/GPU 显存预检/fl2va keyframe 守卫 | E11/E18/E20/E29；108 测试中 ~30 个失败路径 |
...
| Agent Notes 决策记忆 | .agents/notes/{proposed,implemented,rejected}/（31 篇）+ README 格式规范 | 本仓库的 24 轮演进全靠它沉淀 |
| 实验即证据（evidence-driven） | experiments/（events+manifest+eval）+ E1-E29 + leaderboard 基线 | E8/E10 衔接结论；E21 修复路径复现 |
...
24 轮（2026-08-14 → 08-16）每轮 = 差距分析 → 修复/增强 → 测试 → 真实验证
