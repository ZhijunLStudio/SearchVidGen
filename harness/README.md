...
- **E44 复测推翻此结论（见 E44）**：配对设计消除噪声后下游效应
  +7.0 清晰可测——E38 是噪声假阴性；"n≈8-10/臂"的门槛被配对设计取代

### E44：记忆下游效应配对复测——E38 是噪声假阴性（2026-08-16）
E42（裁判 sd=0）+ E43（同种子 MAE=0）消除三层噪声后，用配对设计
（唯一变量 memory.path，同种子/温度 0/无剧本评审，n=1/臂）复测：
- 剧本逐段不同（记忆注入在位，B 臂缺失"段间机位/景别变化"规则）；
  段视频 MAE=39.67（处理效应传播到像素）
- segment 评分两臂均 10/10（段级裁判天花板饱和，呼应 E42）
- **cross_consistency：A=10.0/10.0 passed vs B=跨段一致性 3.0 failed
  ——+7.0 下游效应**。因果链闭合：记忆→剧本（E37 +0.70）→视频差异
  →跨段一致性（+7.0）
- 连带修复：`Experiment.warn()` 公开警告通道 + 记忆文件缺失可见化——
  上线当天即逮住本实验首版任务的路径双重拼接（A 臂险空跑）
- 方法学：配对设计让下游因果复测只需 2 格 ~45 分钟；n=1 为点估计，
  多故事/多种子泛化复测成本已可行

### E45：记忆下游效应第二故事复测——效应是故事条件性的（2026-08-16）
同设计换全新 query（雪山邮差与金毛犬）复测 E44：
- 剧本逐段不同、段视频 MAE=62.28/72.43（处理效应在位），
  但 **cross_consistency 两臂均 10.0/10.0——Δ=0.0**：本故事空臂
  剧本恰好自然保证段间变化，无需记忆规则也通过
- **结论**：+7.0 未复现，记忆的下游价值是**防护性**的（空臂剧本
  产出冻结帧式重复段时防住失败，E44），非普遍加分（E45）——
  E38 的翻案精确化为"下游效应存在，条件于故事是否触发规则"
- 韧性实战：一次 bench 经历外部整树击杀 + 裁判服务被外部击杀
  （GPU7 回收）+ 重启后格级续跑两次 → 完成——E22/E29 续跑纪律
  同日两次兑现
...
### E42：裁判重复评测方差分解——确定性实证 + 跨代不可比警示（2026-08-16）
`scripts/judge_repeatability.py`（口径全部读 run 的 config.yaml 快照，
零硬编码）对同视频重复评测：
- 本代裁判（qwen3.5-27b，temperature 0 + 关思考）固定帧输入 **sd=0**：
  seg01 ×5 = 10.0/10.0（与原记录一致）、蓝屏合成 ×3 = 1.0/1.0
  ——高/低分两端都完全确定性，E16/E23 臂间方差全部来自生成与抽帧，
  不是裁判噪声
- **跨代不可比实证**：8-14 记画面质量 5.0 的 seg04，今日裁判 ×5 全部
  10.0——旧 run 的裁判是启用思考的旧版（同名 adapter 不同模型代），
  8-16 前评分与今日不可比；leaderboard 旧布局推断标注已强化为
  "（推断：旧布局未记录，裁判版本未知，跨期不可比）"
