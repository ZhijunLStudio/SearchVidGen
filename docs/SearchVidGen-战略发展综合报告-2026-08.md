...
# 执行进展（2026-08-14/15，真实实验证据）

> 方向已按"harness"定调落地：`harness/` 目录为可运行代码，以下全部为真实跑通结果。

## 已建成：VidHarness（一切皆插件，对齐 deepseek-harness 哲学）

- **能力缝**：生成(MediaGenerator 全模态) / 剧本(ScriptGenerator) / 评测(Judge) / 转写(Transcriber)
- **提供者**：MiniMax H3 本地（fl2va/t2va 双卡 + ref2va int8 单卡）、MiniMax H3 官方 API（未测，缺 key）、
  DeepSeek V4 Flash（剧本）、vLLM Qwen3.5-27B（多模态裁判）、SenseVoice（音频验证）
- **消费者**：judge_loop（生成→评分→失败反馈重试）、SegmentDirector（跨段编排：首尾帧/参考/无衔接三策略）、
  assemble（FFmpeg 拼接+字幕）、audio_verify（旁白转写比对）
- **实验体系**：manifest/产物缓存/断点续跑(--resume)/成本统计(API+GPU)/HTML 对比报告/8 个单元测试

## 已跑通：端到端「搜索词→成片」×2 部

1. 「程序员深夜加班点亮城市」32s：段评测 9.67/9.0（一次通过）、跨段 10.0/10.0/10.0
2. 「雨夜小猫找到家」37.7s：旁白朗读 4/4 段 ASR 转写与剧本**逐字一致**（重叠度 0.83-1.0）

## 关键实验发现（E1-E8 详见 harness/README.md）

- E2 思考型裁判 token 燃烧 → enable_thinking=false + 别名解析 + 评分纪律
- E3/E8 **画面级连续 ≠ 好的剪辑**：首尾帧硬衔接=冻结帧（叙事推进 1.0/10），文本延续=切镜自由（10.0/10）
- E7 ref2va int8 单卡方案：显存 78GB→38GB（独立环境 torch 2.13+torchao 0.18）
- E4 成本实盘：单段 ~18min(A800×2)，一部 4 段片 GPU 成本 ≈ $1.6-2

## 相对 2026 市场的最新情报修正

- Wan2.5 权重已从 HF/魔搭下架（阿里 API 化）；可下载的最新开源音视频模型 = MiniMax H3（2026-07）
- DeepSeek V4 API 目前不支持图像输入（文本可用）→ 评测必须本地多模态模型
